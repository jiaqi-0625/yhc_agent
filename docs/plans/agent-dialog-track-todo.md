...
| AG-102 | 实现任务切换时的上下文加载与显式提示 | AG-101 | 已实现待集成 | UI 清楚显示当前品牌、车型、项目和任务 |
| AG-103 | 实现跨任务、跨项目和跨账号会话隔离 | AG-101 | 待开始 | 越权恢复会话被后端拒绝并有测试 |
| AG-104 | 实现会话新建、恢复、取消和重置 | AG-101 | 已实现待集成 | 不改变业务状态，取消不产生重复副作用 |
| AG-105 | 为任务上下文生成精简只读摘要 | AG-102 | 已实现待集成 | 包含快照版本和当前阶段，不泄漏无权数据 |

## 里程碑 2：流式事件与 Agent 面板

| ID | 待办 | 依赖 | 状态 | 完成标准 |
|---|---|---|---|---|
| AG-201 | 固化流式事件 Schema | AG-004 | 已实现待集成 | 覆盖消息、思考摘要、工具状态、计划、错误和完成 |
| AG-202 | 保留并迁移现有 Agent 时间线能力 | AG-201 | 已实现待集成 | 当前 `main` 的流式时间线无功能回退 |
...
| AG-302 | 实现营销策略建议能力 | AG-301, WS-101 | 已实现待集成 | 输出通过版本化 Schema，不直接推进状态 |
...
| AG-306 | 限制生产 Agent 工具白名单和结果脱敏 | AG-301 | 已实现待集成 | 未知/通用工具默认拒绝，敏感字段不进入消息 |

## 里程碑 4：操作卡片

| ID | 待办 | 依赖 | 状态 | 完成标准 |
|---|---|---|---|---|
| AG-401 | 生成结构化 `AgentActionCard` | AG-003, AG-302 | 已实现待集成 | 包含 taskId、action、expectedRevision、摘要和成本 |
| AG-402 | 实现操作卡片 UI 与明确的用户执行按钮 | AG-401, AG-203 | 已实现待集成 | Agent 不能自动点击或默认确认 |
