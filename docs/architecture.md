...
已覆盖车型事实、策略、脚本、资产匹配、分镜和交付阶段的只读建议上下文；任务素材快照只在资产匹配经人工确认后可读。WS-305 已提供认证工作区统一命令 API，真实 Agent 面板已挂入三栏 Workspace，并与当前 V2 任务、会话和 revision 同步。WS-210 在显式配置后提供真实 Ark 视频生成、服务端预算/运行锁、MP4 校验、私有对象存储和认证播放纵切；默认仍关闭且自动化测试不产生费用。自动剪辑和最终交付 Provider 尚未接入。
...
按[产品规格第 12.3 节](./workspace-v2-product-spec.md#123-当前持久化边界)，PostgreSQL 是 Workspace V2 生产权威持久化系统；本地 Workspace V2 文件 Store 只用于隔离测试与旧链路兼容。旧 `.data/works` 只保留只读兼容和 WS-307 显式迁移来源，不得作为 V2 Agent 的任务上下文回退。

PostgreSQL 技术方案记录于 [ADR-001](./decisions/adr-001-postgresql-persistence.md)。生产装配显式选择 PostgreSQL，API 进程共享一个连接池，Store 通过同一连接执行 `BEGIN`/`COMMIT`/`ROLLBACK`；控制语句失败会淘汰状态未知的连接。启动只检查连接、迁移 checksum、必需表列和约束，不自动执行 DDL。
...
九个在线 Store 使用 PostgreSQL：Workspace 管理状态、Workspace Session、账号额度、账号高消耗运行锁、批次项目/项目资产池、项目临时资产、视频任务、媒体产物元数据和视频生成请求审计。媒体视频字节不进入 PostgreSQL；数据库保存租户作用域、不可变对象定位符、媒体属性、内容哈希、创建幂等信息，以及实际发送给视频 Provider 的终态不可变 prompt/哈希、输入版本、Provider Job/状态、费用和最终媒体关联。项目资产的跨 Store 服务操作由根事务和 PostgreSQL advisory lock 跨实例串行，同一异步调用链的嵌套 Store 复用该连接并整体提交或回滚；需要新的跨聚合原子写入命令时仍须由显式协调器建立根事务。旧 `.data/works` 只可由 WS-307 显式迁移读取，不能用来恢复或推断 Workspace V2 业务状态；Agent transcript 继续是隔离的本地对话存储。
...
本地测试装配与 PostgreSQL 生产装配都必须从当前 V2 Store 解析认证 Agent 的 `TaskContext`，不得以 legacy `.data/works` 回退来规避失败。WS-307 仍缺 PostgreSQL target adapter；正式数据迁移还必须完成目标适配器、身份、运行恢复和发布验收，不能把数据库 readiness 等同于完整业务可用性。
...
操作卡片发生在 Agent run 完成后的显式人工点击中，因此成功结果不伪装成同一 run 的后续 `AgentStreamEvent`。前端只在严格核验统一命令 response、receipt、账号、项目、任务和 revision 后，生成稳定的 `action_result_<receiptId>` 时间线节点；节点按 event ID 去重，明确标注“业务状态以工作区最新数据为准”，不会成为任务状态或人工确认的权威来源。
...
WS-305 统一命令边界在每次执行时重新校验认证会话、最新授权、项目/任务作用域、当前负责人、revision 和工作流状态，并返回服务端权威成本与持久化 receipt。AG-403 已将 V2 三栏操作卡片切换到该边界；AG-404/AG-405 分别补齐稳定失败状态、防重复提交和成功结果时间线。模型可以请求人工确认，但不能确认自己的产物。
...
- WS-501/WS-502 与 AG-403–AG-405 已完成三栏 Agent、任务上下文、统一命令、失败处理和结果时间线闭环；AG-504/AG-505 的工作区联调也已完成。
