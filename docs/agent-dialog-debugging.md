...
8. 点击测试卡片后，确认卡片变为不可重复提交，并新增“操作结果”时间线节点；节点 revision 应与 Agent `TaskContext` 和工作区任务版本一致，且明确提示业务状态以工作区数据为准。
9. 最后运行 `npm run check`，确认类型检查、全部测试和凭据扫描通过。
...
| 操作卡片提示登录失效或会话不匹配 | 重新登录或切换到有权账号，再重新打开对应任务；不要复用旧会话 |
| 操作卡片提示请求冲突 | 同一请求标识已用于另一项操作；先核对原操作结果，不要更换标识重复提交 |
| 操作卡片提示任务数据或策略需刷新 | 当前任务快照、素材快照或策略依据已经变化；刷新任务并重新获取建议，不要继续提交旧卡片 |
| 卡片成功但没有“操作结果”节点 | 核对 response receipt 是否与当前账号、项目、任务、请求标识和 revision 完全匹配；不能用未核验响应或本地推测补写成功节点 |
...
- WS-501/WS-502 和 AG-403–AG-405 已完成 V2 三栏挂载、上下文隔离、统一命令、稳定幂等、失败展示与成功结果时间线；操作结果节点不替代工作区权威任务状态。
- Agent transcript 仍是隔离的本地对话恢复存储；PostgreSQL 保存 Workspace V2 权威业务状态。不得从 transcript 推断任务 revision、审批、预算或素材事实。
