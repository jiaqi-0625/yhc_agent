...
const webAssets = new Map<string, { path: string; type: string }>([
  ...
  ["/workspace-frame.css", { path: fileURLToPath(new URL("../../web/public/workspace-frame.css", import.meta.url)), type: "text/css; charset=utf-8" }],
...
export async function sendWebAsset(response: ServerResponse, pathname: string): Promise<boolean> {
  ...
  response.writeHead(200, {
    ...
    "content-security-policy": "default-src 'self'; connect-src 'self'; img-src 'self' data: blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
