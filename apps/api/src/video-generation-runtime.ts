...
export class VideoGenerationRuntime {
  ...
  async #auditTerminal(record: Readonly<VideoGenerationRecord>): Promise<void> {
    if (this.generationRequests === undefined || !["succeeded", "failed", "cancelled"].includes(record.status)) return;
    if (
      record.vehicleSnapshotId === undefined
      || record.promptText === undefined
      || record.promptSha256 === undefined
      || record.modelId === undefined
      || record.aspectRatio === undefined
      || record.durationSeconds === undefined
      || record.settledAt === undefined
    ) {
      throw error("AIC-VIDEO-GENERATION-AUDIT-INCOMPLETE", "The terminal generation is missing immutable audit inputs.", 500);
    }
    const success = record.status === "succeeded" && record.output !== undefined;
    const shortestEdge = record.output === undefined ? 1080 : Math.min(record.output.width, record.output.height);
    const resolution: VideoGenerationRequestRecord["resolution"] = shortestEdge <= 480
      ? "480p"
      : shortestEdge <= 720 ? "720p" : "1080p";
    const auditId = `video_generation_request_${createHash("sha256").update(record.id).digest("hex").slice(0, 32)}`;
    await this.generationRequests.create({
      id: auditId,
      tenantId: record.tenantId,
      batchProjectId: record.batchProjectId,
      videoTaskId: record.videoTaskId,
      ...
      taskRevision: record.sourceTaskRevision,
      vehicleSnapshotId: record.vehicleSnapshotId,
      assetSnapshotId: record.assetSnapshotId,
      storyboardArtifactVersionId: record.storyboardArtifactVersionId,
      providerId: record.providerId,
      providerJobId: record.providerJobId,
      providerStatus: record.status,
      outcomeStatus: success ? "succeeded" : "failed",
      modelId: record.modelId,
      resolution,
      aspectRatio: record.aspectRatio as VideoGenerationRequestRecord["aspectRatio"],
      durationSeconds: record.durationSeconds,
      promptText: record.promptText,
      promptSha256: record.promptSha256,
      requestedAt: record.createdAt,
      completedAt: record.settledAt,
      chargedAmountMinor: success ? record.estimatedAmountMinor : 0,
      currency: "CNY",
      ...(success ? { mediaArtifactId: record.output!.artifactId } : {
        failureCode: record.failure?.code ?? `AIC-VIDEO-GENERATION-${record.status.toUpperCase()}`,
      }),
    });
  }
  ...
  async start(projectId: string, videoTaskId: string, input: Readonly<StartVideoGenerationInput>, session: Readonly<WorkspaceSessionScope>) {
    ...
    if (!task.videoTask.vehicleSnapshotId || !task.videoTask.assetSnapshotId || !task.videoTask.scriptInput) {
      throw error("AIC-VIDEO-GENERATION-INPUT-NOT-READY", "The locked vehicle facts, assets, and confirmed script are required before video generation.", 409);
    }
    ...
    if (replay) {
      ...
      await this.#auditTerminal(replay);
    ...
    try {
      ...
      const initial: VideoGenerationRecord = {
        ...
        vehicleSnapshotId: task.videoTask.vehicleSnapshotId,
      ...
      if (createdRecord) {
        ...
        const failed = await this.jobs.save({
          ...createdRecord,
          status: "failed",
        ...
        if (failed !== undefined) await this.#auditTerminal(failed).catch(() => undefined);
      }
      throw cause;
    }
  }
  ...
  async #applyProviderJob(current: Readonly<VideoGenerationRecord>, providerJob: Readonly<VideoGenerationJob>): Promise<VideoGenerationRecord> {
    ...
    const saved = await this.jobs.save(next, current.revision);
    await this.#auditTerminal(saved);
    return saved;
  }
  ...
  async get(projectId: string, videoTaskId: string, jobId: string, session: Readonly<WorkspaceSessionScope>) {
    ...
    if (record.status === "succeeded" || record.status === "failed" || record.status === "cancelled") {
      await this.#auditTerminal(record);
