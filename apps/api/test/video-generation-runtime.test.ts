...
const task = {
  videoTask: {
    ...
    vehicleSnapshotId: "vehicle_snapshot_locked",
...
test("real video generation requires a human start then persists and polls the provider job", async () => {
  ...
  let released = 0;
  let charged = 0;
  const audits: VideoGenerationRequestRecord[] = [];
  ...
  const auditStore = {
    async create(record: Readonly<VideoGenerationRequestRecord>) {
      const saved = structuredClone(record);
      audits.push(saved);
      return { record: saved, replayed: false };
    },
  } as unknown as VideoGenerationRequestStore;
  const runtime = new VideoGenerationRuntime(
    administration,
    projects,
    tasks,
    new MemoryVideoGenerationStore(),
    provider,
    runLocks,
    budgets,
    undefined,
  ...
  assert.equal(audits[0]?.promptSha256.length, 64);
  assert.match(audits[0]?.promptText ?? "", /中文女声旁白/u);
  assert.equal(audits[0]?.vehicleSnapshotId, "vehicle_snapshot_locked");
  assert.equal(audits[0]?.mediaArtifactId, "artifact_1");
});
