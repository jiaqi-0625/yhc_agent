...
test("policy exposes the current strategy draft only during strategy review states", () => {
  for (const status of ["strategy_draft", "awaiting_strategy_approval", "strategy_approved"] as const) {
    assert.deepEqual(
